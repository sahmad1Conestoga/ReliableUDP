...
#include <unistd.h>
	...
	public:
		...
		bool Send(const Address& destination, const void* data, int PacketSizeHack)
		{
			assert(data);
	...
	public:
		...
		virtual bool SendPacket(const unsigned char data[], int PacketSizeHack)
		{
			...
			return socket.Send(address, &packet[0], PacketSizeHack + 4);
		}
		...
		virtual int ReceivePacket(unsigned char data[], int PacketSizeHack)
		{
			...
			std::vector<unsigned char> packet(PacketSizeHack + 4);
			Address sender;
			int bytes_read = socket.Receive(sender, &packet[0], PacketSizeHack + 4);
			if (bytes_read == 0)
				return 0;
			if (bytes_read <= 4)
				return 0;
